...
def generate_splituser(X, Y, num_samples):
    ...
    for i in range(NUM_USER):
        ...
        yy = Y[i * samples_per_user[i]: (i + 1) * samples_per_user[i]]
...
def run():
    ...
    path = "/media/Research/linkaixi/AllData/federatedlearning/w8a/"
    # path = "./"
    ...
    X_train, Y_train = generate_splituser(X, Y, N)

    X, Y, N = load_data(test_file, dimension)
    X_test, Y_test = generate_splituser(X, Y, N)
