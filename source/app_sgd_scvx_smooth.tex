...
\begin{proof}
	...
	\begin{align*}
	\frac{1}{K}\sum_{k}p^{k}\|w_{t}^{k}-\ov{w}_{t}\|^{2} & =O(\alpha_{t}^{2}G^{2}/K)\\
	\sum_{k}p^{k}\|w_{t}^{k}-\ov{w}_{t}\|^{2}\le\sum_{k}p^{k}\|w_{t}^{k}-\ov{w}_{t_{0}}\|^{2} & \leq\sum_{k}p^{k}\|-\alpha_{t_{0}}g_{t_{0},k}-\alpha_{t_{0}+1}g_{t_{0}+1,k}-\cdots-\alpha_{t-1}g_{t-1,k}\|^{2}\\
