...
local devices is active at each communication round. In this work,
...
In the partial participation scenario, $\cS_{t+1}$ is obtained by two types of
sampling schemes to simulate the practical scenarios~\cite{li2019convergence}. The first one samples
each device with probability $p_k$, with replacement. The second one samples
each device uniformly without replacement. Both of these sampling schemes
guarantee that gradient updates in FedAvg are unbiased stochastic versions of
updates in FedAvg with full participation, during the communication round. 
