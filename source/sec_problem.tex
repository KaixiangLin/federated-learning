...
\textbf{Justfication for Assumptions~\ref{ass:boundedvariance} and~\ref{ass:subgrad2}.} These standard assumptions have been made in many previous
...
hand, some works such as \cite{haddadpour2019convergence} assume
bounds on the ``gradient diversity'', defined as $\frac{\sum_{k}p_{k}\|\nabla F_{k}(\mathbf{w})\|_{2}^{2}}{\|\sum_{k}p_{k}\nabla F_{k}(\mathbf{w})\|_{2}^{2}}$.
Such assumptions are more restrictive than our assumptions, because
they imply $\nabla F_{k}(\mathbf{w}^{\ast})\equiv0$ for all local
objectives. For such ``overparameterized'' settings, we also prove
that the convergence is in fact exponential. 
