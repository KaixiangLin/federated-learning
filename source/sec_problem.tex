...
We make the following standard assumptions on the objective function $F_1,\dots, F_N$. Assumption~\ref{ass:lsmooth} and Assumption~\ref{ass:stroncvx} are common assumptions satisfied by a range of popular objective functions, such as $\ell^{2}$ regularized logistic regression and cross-entropy loss functions.
...
\begin{assumption}[Bounded local gradient]
	...
	\label{ass:subgrad2}
\end{assumption}
These standard assumptions have been made in many previous works in federated learning\cite{yu2019parallel,li2019convergence,stich2018local}. 
Here we provide one more justification
...
and assuming the bounds at $\mathbf{w}^{\ast}$ only. Furthermore, instead of  
assuming \textit{bounded gradient diversity} as in related works~\cite{haddadpour2019convergence,li2018federated}, 
we argue that Assumption~\ref{ass:subgrad2} is more general. Because when converging to target accuracy $\epsilon$ and $\epsilon \rightarrow 0$,
bounded gradient diversity restricts local objectives to the same minimizer as global objective, which contradicts to the heterogeneous data setting, 
while our assumption is still feasible.
For more detailed explanations of our assumptions, please refer to Appendix Section~\ref{sec:app:comparison}.
