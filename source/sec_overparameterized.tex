...
improved convergence rate over FedAvg. The proof is deferred to Appendix Section~\ref{sec:interpolation}.
\begin{comment}
...
the speedup factor is on the order of $\mathcal{O}(N/E)$ %\frac{N}{E(l\nu_{\max}+L(N-\nu_{\min}))}/\frac{1}{l\nu_{\max}+L(1-\nu_{\min})}\approx\frac{Nl}{E(l+L(N-1))}=
