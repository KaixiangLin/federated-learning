...
& \lkxcom{\leq \EE\|\vw_t^k  - \ov{w}_{t_0}\|^2 } \\
...
\end{align}
where the second line is same from Lemma 3~\cite{li2019convergence} \lkxcom{needs to be verified.} $\ov{w}_{t_0} = \vw^k_{t_0}$ and $t_0$ is the step we communicate. In the third line, we use the \eq{\ref{eq:nasgdzt}}. In fourth line, we use \eq{\ref{eq:recursivep}}. 
...
We now bound $C_1$, we set $\Lambda_{t_0} = \sum_{j=0}^{t_0 - 1}\beta^{t_0-1-j} $
\begin{align}
	C_1 &=  \frac{\beta^4(2\beta^{t-t_0}-1)^2 + 1)}{(1-\beta)^2} \EE\|\sum_{j=0}^{t_0 - 1}\beta^{t_0-1-j}\vg_j\|^2\\
        ...
        &= \eta_t^2  \frac{\beta^4(2\beta^{t-t_0}-1)^2 + 1)}{\alpha^2}\Lambda_t^2 \EE\|\sum_{j=0}^{t_0 - 1}\beta^{t_0-1-j}/\Lambda_t \vg_j\|^2\\
        & \leq \eta_t^2  \frac{\beta^4(2\beta^{t-t_0}-1)^2 + 1)}{\alpha^2}\Lambda_t^2 \sum_{j=0}^{t_0 - 1}\beta^{t_0-1-j}/\Lambda_t\EE\| \vg_j\|^2\\ 
        & = \eta_t^2 \Lambda_t \frac{\beta^4(2\beta^{t-t_0}-1)^2 + 1)}{\alpha^2} \sum_{j=0}^{t_0 - 1}\beta^{t_0-1-j} \EE\| \vg_j\|^2\label{eq:nagcvxc1}
\end{align}
In the fourth line we use the Jensen's inequality. 
...
\begin{align}
 ...
 \leq & 2\sum_{k=1}^K p_k  (E-1)4\eta_{t}^2 \sum_{j=t_0}^{t-1} \EE \| \vg_{j,k}\|^2 \leq 8(E-1)\eta_{t}^2 \sum_{k=1}^K p_k \sum_{j=t_0}^{t-1} \EE \| \vg_{j,k}\|^2\label{eq:nagcvxc2}
\end{align}
To bound $C_3$, similar to $C_1$, we set $\Lambda_{t_0}^{t} = \sum_{j=t_0}^{t - 1}\beta^{t_0-1-j} $
\begin{align}
	C_3 & = 2\sum_{k=1}^K p_k \frac{\beta^4}{(1 - \beta)^2} \EE\|\sum_{j=t_0}^{t-1}\beta^{t-j-1}\vg_{j,k}\|^2\\
        & = 2\eta_t^2 \Lambda_{t_0}^{t}\sum_{k=1}^K p_k \frac{\beta^4}{\alpha^2} \sum_{j=t_0}^{t-1}\beta^{t-j-1} \EE\|\vg_{j,k}\|^2\label{eq:nagcvxc3}
\end{align}
