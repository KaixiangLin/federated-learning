...
\textit{From full to partial participation.} In addition to the variances arising from stochastic updates at local clients and averaging across clients, partial participation, which involves a sampling procedure, gives rise to a third variance term. An additional term $\mathcal{O}(\frac{1}{K} \alpha_t^2 E^2G^2)$ of leading order resulting from the sampling variance is added to the recursive contraction bound.
