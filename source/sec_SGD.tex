...
\begin{align*}
	\mathbb{E}\|\ov{w}_{t+1}-\vw^{\ast}\|^{2} & \leq (1-\alpha_t)\mathbb{E}\|\ov{w}_{t}-\vw^{\ast}\|^{2}+ \cO(\alpha_t^2 \sigma^2/N + \alpha_t^3E^2LG^2).
\end{align*}
...
\textit{From full to partial participation.} In addition to the variances arising from stochastic updates at local clients and averaging across clients, partial participation, which involves a sampling procedure, gives rise to a third type of variance: an additional term $\mathcal{O}(\frac{1}{K} \alpha_t^2 E^2G^2)$ of leading order resulting from the sampling variance is added to the recursive contraction bound.
...
\begin{align*}
 ...
 \leq &\mathbb{E}\|\ov{w}_{t}-\vw^{\ast}\|^{2}+\cO(\alpha_t^2 \nu_{\max}^{2} \sigma^2/N + \alpha_t^3E^2LG^2).
	\end{align*}
